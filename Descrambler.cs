    ...
    internal static class Descrambler
    {
        ...
        public static string Descramble(Stream stream)
        {
            ...
            if (magic[0] != 0xFE || magic[1] != 0xFE)
                throw new InvalidDataException("Not a scrambled Kirikiri file.1");

            byte mode = reader.ReadByte();
            byte[] bom = reader.ReadBytes(2);
            if (bom[0] != 0xFF || bom[1] != 0xFE)
                throw new InvalidDataException("Not a scrambled Kirikiri file.2");
            byte[] utf16;
            switch (mode)
            {
                case 0:
        ...
        private static byte[] Decompress(BinaryReader reader)
        {
            ...
            using (DeflateStream deflate = new DeflateStream(reader.BaseStream,
                                                             ...
                                                             true))
            {
                ...
                {
                    int read = deflate.Read(uncompressedData, totalRead,
                                           (int)(uncompressedLength - totalRead));
                    if (read == 0)
                        break;
